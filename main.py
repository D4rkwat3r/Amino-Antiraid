...
async def run_modules(
    ...
    for module in config.modules:
        ...
        if issubclass(module_class, module_classes.PollingModule):
            try:
                polling_process_class = signature(module_class.__init__).parameters["polling_process"].annotation
            except KeyError:
                log.error(__name__, f"Модуль {module.name} не имеет в методе __init__ параметра polling_process")
                continue
            if not issubclass(polling_process_class, polling_process_classes.PollingProcess):
                log.error(__name__, f"Модуль {module.name} имеет неверную (или не имеет "
                                    f"вообще) аннотацию типа у параметра polling_process. "
