...
class ApiClient:
    ...
    async def send_message(self, ndc_id: int, thread_id: str, content: str, message_type: int = 0) -> bool:
        return await self.authenticate_and_send(
    ...
    async def comment_publication(self, ndc_id: int, publication_type: int, publication_id: str, content: str) -> bool:
        return await self.authenticate_and_send(
            ApiRequest.post(f"{'item' if publication_type == 1 else 'blog'}/{publication_id}/comment")
            .community_scope(ndc_id)
            .param("content", content)
            .param("mediaList", [])
            .param("stickerId", None)
            .param("type", 0)
            .param("eventSource", "PostDetailView")
        )

    async def comment_profile(self, ndc_id: int, user_id: str, content: str) -> bool:
        return await self.authenticate_and_send(
            ApiRequest.post(f"user-profile/{user_id}/comment")
            .community_scope(ndc_id)
            .param("content", content)
            .param("mediaList", [])
            .param("stickerId", None)
            .param("type", 0)
            .param("eventSource", "UserProfileView")
        )
