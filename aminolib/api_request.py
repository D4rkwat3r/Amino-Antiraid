...
class ApiRequest:
    ...
    url: str = "https://service.narvii.com/api/v1"
    ...
    def generate_message_signature(data: bytes) -> str:
        return b64encode(
            bytes.fromhex("42") + new(bytes.fromhex("F8E7A61AC3F725941E3AC7CAE2D688BE97F30B93"), data, sha1).digest()
        ).decode("utf-8")

    @staticmethod
    def generate_device_id() -> str:
        identifier = urandom(20)
        return f"42{identifier.hex()}" \
               f"{new(bytes.fromhex('02B258C63559D8804321C5D5065AF320358D366F'), bytes.fromhex('42') + identifier, sha1).hexdigest()}".upper()
