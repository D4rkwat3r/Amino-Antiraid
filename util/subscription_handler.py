...
class SubscriptionHandler:
    ...
    def subscribe(
        ...
        self._subscriptions.append({"id": subscription_id, "handler": handler, "filter": content_filter})
    ...
    def broadcast(self, content: Any):
        for subscription in self._subscriptions:
            if not subscription["filter"](content):
                continue
            create_task(subscription["handler"](content)) \
                if iscoroutinefunction(subscription["handler"]) \
                else subscription["handler"](content)
